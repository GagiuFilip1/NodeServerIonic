...
router.get('/', async ctx => {
  ...
  response.body = await _store__WEBPACK_IMPORTED_MODULE_1__["default"].findAll({
    userId
  });
  response.status = 200; // ok
});
router.post('/pagination', async ctx => {
  console.log(ctx.request.body.skip);
  console.log(ctx.request.body.length);
  const response = ctx.response;
  const userId = ctx.state.user._id;
  response.body = await _store__WEBPACK_IMPORTED_MODULE_1__["default"].findPagination({
    userId
  }, ctx.request.body.skip, ctx.request.body.length);
  response.status = 200; // ok
});
